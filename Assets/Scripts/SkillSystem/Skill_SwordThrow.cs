...
public class Skill_SwordThrow : Skill_Base
{
    ...
    {
        base.Awake();
        swordGravity = swordPrefab.transform.GetComponent<Rigidbody2D>().gravityScale;
        dots = GenerateDots();
    }
    ...
    private GameObject GetSwordPrefab()
    {
        if (Unlocked(SkillUpgradeType.SwordThrow))
            return swordPrefab;
        if (Unlocked(SkillUpgradeType.SwordThrow_Piece))
    ...
    private void UpdateThrowPower()
    {
        switch (upgradeType)
        {
            ...
            case SkillUpgradeType.SwordThrow_Spin:
                ...
                break;
            case SkillUpgradeType.SwordThrow_Piece:
    ...
    private Vector2 GetTrajectoryPoint(Vector2 direction, float t)
    {
        ...
        Vector2 gravitiyEffecty = 0.5f * Physics2D.gravity * swordGravity * (t * t);

        // We calculate how far the sword will travel after time 't',
        // by combining the initial throw direction with the gravity pull.
        Vector2 predictedPoint = (initialVelocity * t) + gravitiyEffecty;
